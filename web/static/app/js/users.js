...
function edit_user_data(e) {
	...
	if (form.attr('data-value') == 'edit_button') {
		form.html('<i class="fa fa-save"></i> Save')
		form.attr('data-value', 'save_button')
		...
		if (validate_company(row)){  // Could be added other validation functions
			...
			form.attr('data-value', 'edit_button')
		}
		else
			alert('Wrong data');
	}
...
function generate_users(e){
	...
	$.ajax({
			...
			success: function(data){
				$('#user_message').append("<h4 style='color:red;'>"+ data.message + "</h4>")
			},
			error: function(e){
				$('#user_message').append("<h4>Something wrong :( </h4>")
			},
		});
}

function update_row_user(user, row) {
